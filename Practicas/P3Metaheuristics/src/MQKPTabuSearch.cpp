...
void MQKPTabuSearch::run(MQKPStopCondition &stopCondition)
{
	...
	while (!stopCondition.reached())
	{
		...
		for (unsigned i = 0; i < numObjs; i++)
		{
			...
			if (this->_shortTermMem_aux.end() == this->_shortTermMem_aux.find(indexObj))
			{
				...
				for (unsigned j = 0; j <= numKnapsacks; j++)
				{
					...
					double deltaFitness = MQKPEvaluator::computeDeltaFitness(*this->_instance, *this->_solution, indexObj, (int)j);
		...
		//TODO Aplicar la operación y almacenarla en la memoria a corto plazo DONE
		bestOperation.apply(*this->_solution);
		this->_shortTermMem.push(bestOperation.getObj());
		_shortTermMem_aux.insert(bestOperation.getObj());

		//TODO Si hay demasiados elementos en la memoria, según la tenencia tabú, eliminar el más antiguo DONE
		if (this->_shortTermMem.size() > this->_tabuTennure)
		{
			unsigned value = this->_shortTermMem.front();
			this->_shortTermMem.pop();
			this->_shortTermMem_aux.erase(value);
		}
