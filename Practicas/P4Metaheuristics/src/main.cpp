...
#include "include/MQKPGeneticAlgorithm.h"
#include "include/MQKPAntColonyOpt.h"
#include "include/MQKPStopCondition.h"
...
//	vector<unsigned> printColumns = { 2, 3, 4, 7, 8};
	vector<unsigned> printColumns = { 5, 6};
	...
	while (allResultsPrinted == false) {
		...
		indexColumn = 0;
				...
				for (auto indexColumnPerInstance : printColumns) {
				cout << "Column " << indexColumnPerInstance << ": " << allTheResults.at(0)->at(indexColumnPerInstance)->size() << endl;
