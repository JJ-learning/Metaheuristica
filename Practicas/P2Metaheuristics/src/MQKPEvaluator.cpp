										  ...
										  MQKPSolution &solution, int indexObject, int indexKnapsack)
{
	...
	double maxViolation = instance.getMaxCapacityViolation(solution);												//Obtenemos la máxima violacion actual
	double deltaMaxCapacityViolation = instance.getDeltaMaxCapacityViolation(solution, indexObject, indexKnapsack); //Obtenemos la capacidad
	double MaxViolationOperation = maxViolation + deltaMaxCapacityViolation;										//Suma de las medidas anteriores
	double actualSumProfits = instance.getSumProfits(solution);														//Obtenemos la suma de beneficios actual
	double deltaSumProfits = instance.getDeltaSumProfits(solution, indexObject, indexKnapsack);						//Obtenemos la suma de beneficios
	double newSumProfits = actualSumProfits + deltaSumProfits;														//Suma de las medidas anteriores

	if (maxViolation > 0 and MaxViolationOperation > 0)
	{
		return (deltaMaxCapacityViolation * -1);
	}

	else if (maxViolation == 0 and MaxViolationOperation == 0)
	{
	...
	return ((actualSumProfits * -1) + (MaxViolationOperation * -1));
}
