...
void MQKPInstance::randomPermutation(int size, vector<int> &perm)
{
	...
	perm.clear(); // Clear vector

	// Fill vector with permutations
										...
										int indexKnapsack)
{
	...
	if (solution.whereIsObject(indexObject) != 0)
	{
		...
		{
			// Both object are within the bag
	 ...
	 */
	// If object is not wihtin any bag
	if (indexKnapsack != 0)
	{
		for (int i = 0; i < this->getNumObjs(); i++)
		{
			// If the object is within the given bag
			if (solution.whereIsObject(i) == indexKnapsack)
			{
				deltaSumProfits += this->getProfit(indexObject, i);
			}
		}
												  ...
												  int indexObject, int indexKnapsack)
{
	 ...
	 */
	int aux_bag;
	double delta_cap_violation;

	aux_bag = solution.whereIsObject(indexObject);										 // Get the object's bag
	delta_cap_violation = this->getMaxCapacityViolation(solution);						 // Get max violation
	solution.putObjectIn(indexObject, indexKnapsack);									 // Set the object in the new bag
	delta_cap_violation = this->getMaxCapacityViolation(solution) - delta_cap_violation; // Get the difference
	solution.putObjectIn(indexObject, aux_bag);											 // Set the object again in the old bag

	return delta_cap_violation;
}
